...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Orbiting dot echoing the logo mark */}
      ...
      <motion.div style={{ opacity }} className="relative z-10 max-w-[1400px] mx-auto px-5 md:px-8 pt-40 md:pt-48 pb-24 min-h-[100vh] flex flex-col justify-center">
        {/* Small badge */}
        <motion.div
        ...
        {/* Main heading */}
        <h1 className="font-display font-semibold text-white text-[28px] sm:text-[42px] md:text-[52px] lg:text-[64px] leading-[1.1] tracking-[-0.02em] max-w-[20ch]" data-testid="hero-heading">
          <div>
            <WordReveal text="Trusted M DOT IT SOLUZIONE" delay={0.25} />
          </div>
          <div className="mt-2">
            <WordReveal text="refurbished laptop" delay={0.45} className="brand-gradient-text" />
          </div>
          <div className="mt-2">
            <WordReveal text="suppliers in India" delay={0.72} />
          </div>
        </h1>
        ...
        >
          Premium refurbished Dell, HP, Lenovo laptops and enterprise IT hardware — 
          certified quality, tested by engineers, delivered across India at unbeatable prices.
        </motion.p>
        ...
        >
          <Link to="/request-quote" className="btn-primary" data-testid="hero-cta-primary">
            Request a quote
            <ArrowUpRight size={18} />
          </Link>
          <a href={`tel:${COMPANY.phoneRaw}`} className="btn-ghost" data-testid="hero-cta-secondary">
            <PhoneCall size={16} />
            {COMPANY.phone}
          </a>
        </motion.div>
      ...
      >
        <span>Scroll</span>
        <motion.div animate={{ y: [0, 6, 0] }} transition={{ duration: 1.8, repeat: Infinity }} className="w-px h-8 bg-white/40" />
      </motion.div>
    </section>
  );
}
